...
public class QuorumHierarchical_C2 implements QuorumVerifier {

    private static final Logger LOG = LoggerFactory.getLogger(QuorumHierarchical_C2.class);
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof QuorumHierarchical_C2)) {
            return false;
        }
        QuorumHierarchical_C2 qm = (QuorumHierarchical_C2) o;
        ...
        return checkMembersEqual(qm) 
            && serverWeight.equals(qm.serverWeight)
            && groupWeight.equals(qm.groupWeight)
            && serverGroup.equals(qm.serverGroup);
    }

    private boolean checkMembersEqual(QuorumHierarchical_C2 qm) {
        for (Map.Entry<Long, QuorumServer> entry : allMembers.entrySet()) {
            QuorumServer qso = qm.getAllMembers().get(entry.getKey());
            if (qso == null || !entry.getValue().equals(qso)) {
                return false;
            }
        }
        return true;
    ...
    public QuorumHierarchical_C2(String filename) throws ConfigException {
        readConfigFile(filename);
    }

    public QuorumHierarchical_C2(Properties qp) throws ConfigException {
        parse(qp);
    ...
    private void readConfigFile(String filename) throws ConfigException {
        ...
        if (!configFile.exists()) {
            throw new ConfigException(configFile.toString() + " file is missing");
        }
    ...
    private void parse(Properties quorumProp) throws ConfigException {
        for (Map.Entry<Object, Object> entry : quorumProp.entrySet()) {
            parseProperty(entry.getKey().toString(), entry.getValue().toString());
        }
    ...
    private void parseProperty(String key, String value) throws ConfigException {
    ...
    private void parseServer(String key, String value) throws ConfigException {
        ...
        if (value.toLowerCase().contains("observer")) {
    ...
    private void validateAndAssignWeights() throws ConfigException {
        for (Long id : participatingMembers.keySet()) {
    ...
    public String toString() {
        ...
        for (Map.Entry<Long, QuorumServer> entry : allMembers.entrySet()) {
            String key = "server." + entry.getKey();
        ...
        for (Map.Entry<Long, Long> pair : serverGroup.entrySet()) {
        ...
        for (Map.Entry<Long, String> pair : groups.entrySet()) {
        ...
        for (Map.Entry<Long, Long> pair : serverWeight.entrySet()) {
            Long sid = pair.getKey();
    ...
    private void computeGroupWeight() {
        for (Map.Entry<Long, Long> entry : serverGroup.entrySet()) {
    ...
    public boolean containsQuorum(Set<Long> set) {
        ...
        for (Map.Entry<Long, Long> entry : expansion.entrySet()) {
